 ...
 * @author user
 */
public class OwnedTeamServ extends HttpServlet {
    @EJB
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet OwnedTeamServ</title>");            
            out.println("</head>");
            out.println("<body>");
            for(int i=0;i<equipeFacade.findAll().size();i++){
                
                Equipe equipe = equipeFacade.findAll().get(i);
                User user = equipe.getIdAdmin();
                equipe_id_admin = user.getId().toString();
                credential = equipe_id_admin.equals(id_admin);
                if(credential==true){
               out.println("<form action=\"/poco_team-war/adminchat.jsp\" >");    
